...
> * 避免作用域被污染
>
> gulp原理是使用IIFE来对作用域进行封装。避免脚本污染全局变量
>
> * 脚本文件模块化，不必全部加载
>
> 使用node.js的commonjs模块化，但是浏览器不支持
>
> * 如何浏览器实现模块化
>
 ~~~javascirpt 
...
> 相关文件名被保存在缓存中，一旦检测到文件名发生变化就会相应的进行更新数据
>
> Cache-Control
...
//在配置文件中进行设置
“scripts”:{
	"build":"rm -rf dist && webpack --config webpack.config.pord.js"
}
~~~
...
* 对相同配置属性进行抽取，配置到文件webpack.config.base.js中

* ~~~
  const base = require(base.js相对路径'')
  ~~~
